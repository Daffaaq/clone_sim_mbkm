...
class LogBimbinganInstrukturController extends Controller
{
    ...
    public function updateStatusInstruktur(Request $request)
    {
        ...
        LogModel::create([
            ...
            'action' => 'update',
            'url' => $this->menuUrl,
            'data' => 'Tanggal: ' . $logBimbingan->tanggal_status_instruktur . ', Status: ' . $status,
            'created_by' => auth()->id(),
    ...
    public function updateStatusInstrukturFromModal(Request $request, $id)
    {
        ...
        $status = ($statusInstrukturLapangan == 1) ? 'Diterima' : (($statusInstrukturLapangan == 2) ? 'Ditolak' : 'Pending'); // Tentukan teks status berdasarkan nilai statusDosen

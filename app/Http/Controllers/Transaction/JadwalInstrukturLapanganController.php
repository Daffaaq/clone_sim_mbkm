...
class JadwalInstrukturLapanganController extends Controller
{
    public function __construct()
    {
        $this->menuCode  = 'TRANSACTION.JADWAL.SEMHAS.INSTRUKTUR';
        $this->menuUrl   = url('transaksi/jadwal-semhas-instruktur');
        $this->menuTitle = 'Jadwal Semhas Instruktur Lapangan';
        $this->viewPath  = 'transaction.jadwal-semhas-instruktur.';
    }
    ...
    public function index()
    {
        ...
        $breadcrumb = [
            ...
            'list'  => ['Instruktur Lapangan', 'Jadwal Semhas Instruktur Lapangan']
        ];

        $activeMenu = [
            'l1' => 'transaction',
            'l2' => 'transaksi-ujasemin',
            'l3' => null
        ];
        ...
        $data = SemhasDaftarModel::where('t_semhas_daftar.periode_id', $activePeriods)
            ...
            ->get();
        $semhasDaftarIds = $data->pluck('semhas_daftar_id'); // Ambil semua semhas_daftar_id sebagai array
        // dd($semhasDaftarIds);

        $existingNilai = TNilaiInstrukturLapanganModel::whereIn('semhas_daftar_id', $semhasDaftarIds)
            ->where('periode_id', $activePeriods)
            ->get();
    ...
    public function simpanNilai(Request $request)
    {
        ...
        $nilaiData = $request->input('nilai');
        $nilaiPembimbingDosenIds = $request->input('nilai_instruktur_lapangan_id');
        $periodeId = $request->input('periode_id');
        $semhasDaftarId = $request->input('semhas_daftar_id');
        ...
        foreach ($nilaiPembimbingDosenIds as $index => $id) {
            ...
            $existingNilai = TNilaiInstrukturLapanganModel::where('nilai_instruktur_lapangan_id', $id)
            ...
            if ($existingNilai) {
                ...
                TNilaiInstrukturLapanganModel::create([
                    'nilai' => $nilaiData[$index],
                    'nilai_instruktur_lapangan_id' => $id,
                    'periode_id' => $periodeId,
                    'semhas_daftar_id' => $semhasDaftarId
                ]);
            }
        ...
        $existingNilai = RevisiInstrukturLapanganModel::where('periode_id', $periodeId)
            ->where('semhas_daftar_id', $semhasDaftarId)
            ->first();

        if ($existingNilai) {
            $existingNilai->update([
                'saran_instruktur_lapangan' => $request->input('saran_instruktur_lapangan'),
                'catatan_instruktur_lapangan' => $request->input('catatan_instruktur_lapangan'),
                // update other fields as needed
            ]);
        } else {
            $saranKomentar = RevisiInstrukturLapanganModel::create([
                'saran_instruktur_lapangan' => $request->input('saran_instruktur_lapangan'),
                'catatan_instruktur_lapangan' => $request->input('catatan_instruktur_lapangan'),
                'semhas_daftar_id' => $semhasDaftarId,
                'periode_id' => $periodeId,
